...
const FaqItem = ({ item, index }) => {
  ...
  return (
    <div className="relative z-2 mb-16">
      <div
        className="group relative flex cursor-pointer items-center justify-between gap-10 px-7 z-2"
        onClick={() => {
          setActiveId(activeID === item.id ? null : item.id);
        }}
      >
        <div className="flex-1">
          <div className="small-compact mb-15 text-p3 max-lg:hidden">
            {index < 10 ? "0" : ""}
            {index}
          </div>
          <div
      ...
      <div
        className={clsx(
          "absolute -z-1 g5 -bottom-7 -top-7 left-0 right-0 rounded-3xl opacity-0 transition-opacity duration-500", active && 'opacity-100'
        )}
      >
